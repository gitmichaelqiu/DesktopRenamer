    ...
    static func moveWindowToSpace(windowID: Int, fromSpaceID: Int, targetSpaceID: Int) {
        ...
        if let targetDisplay = targetDisplayID,
           ...
           targetDisplay != sourceDisplay {
            // A cross-display move must begin with the target display on the
            // destination space. Otherwise Mission Control can accept the CGS
            // assignment but leave the window visually attached to the old
            // display's active space.
            let targetIsActive = getCurrentSpaceID(for: targetDisplay) == targetSpaceIDString
            if !targetIsActive {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
                    guard getCurrentSpaceID(for: targetDisplay) == targetSpaceIDString else {
                        DiagnosticEventLog.shared.record(
                            ...
                            level: "error",
                            "Could not activate target display space \(targetSpaceIDString) before moving window \(windowID)."
                        )
                        return
                    }
